...
void Qwt3D::Plot3D::drawPoints() {
  ...
  //  gluQuadricTexture(quadratic, GL_TRUE); // Create Texture Coords ( NEW )
    ...
    for(int i=0;i<this->data_wek.size();i++)
    {
        glVertex3d(this->data_wek[i]->x, this->data_wek[i]->y, this->data_wek[i]->z);
    }

    //gluSphere(quadratic,0.07f,32,32);
